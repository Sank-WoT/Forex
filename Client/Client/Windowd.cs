...
<<<<<<< HEAD
        ...
        /// param name="Vvalue" Наименование котировки
        /// </summary>
...
<<<<<<< HEAD
            ...
            graphic.Series[4].Points.AddXY(DateT[0].ToOADate(), Buffer[0]);
            
            // каждый U[date перестраивать таблицу?
...
<<<<<<< HEAD
                ...
                label_X.Location = new Point(Convert.ToInt32(110 * xS * (WString.X / fX)), e.Y); 
                // перемещение линии по Y
                label_Y.Location = new Point(e.X, Convert.ToInt32(30 * yS * (WString.Y / fY)));
                // привязка значения 
                lab_Cur.Location = new Point(Convert.ToInt32(700 * xS * (WString.X / fX)), e.Y);
                ...
                // перевод времени указателя
                lab_Cur.Text = string.Concat(string.Concat(Math.Round(YCur, 5).ToString(), " , "), Date1.ToString("h:mm:ss.fff tt")); 
=======
...
<<<<<<< HEAD
                ...
                // Путь к файлу c котировками eurusd
                string pathFile = pathDirectory + "\\" + value + ".txt";
                ...
                if (tic == 0)
                {
                    ...
                    rEURUSD.read(textRead, massYInetBuy, massYInetSell, Times); 

                    DINET = cEURUSD.ConvertD(Times); // Конвертируем время из  формата UNIX в DataTime
                } 
            }
            // Текущее время с учетом задержек
            double dTime = (DateTime.Now - new DateTime(1970, 1, 1, 0, 0, 0, 0)).TotalSeconds - 15;
            // текущее время UnitTime            
            int NowTime = Convert.ToInt32(dTime);
            Secon(NowTime);
            //  посекундные запросы к сайту
            return NowTime;
        }

        public void Secon(int NowTime)
        {
            if (InetConnect.Inet == true)
            {
                // интернет запрос текущего времени
                InternetRequest Request = new InternetRequest(NowTime, 1, value);
                // коллекция распарсенных данных
                MatchCollection RequestSet;
                // присвоение результата запроса
                RequestSet = Request.InternetData(); 

                // Присвоение к последнему числу в записи для обработки данных при отсутствии интернета.
=======
...
>>>>>>> parent of bcab9ee... Forex2.0 Переписать код под более строгую архитектуру ООП, создать БД.
                {
                    // проверка
                    Console.WriteLine(Convert.ToDouble(RequestSet[1].Value));
                    // добавить в лист значения покупки
                    Buffer.Add(Convert.ToDouble(RequestSet[1].Value));
                    // добавить в лист значения продажи       
                    BufferS.Add(Convert.ToDouble(RequestSet[2].Value));
                    // последнее число в покупке   
                    poslchislo = Convert.ToDouble(RequestSet[1].Value);
                    // последнее число в продаже
                    poslchislo1 = Convert.ToDouble(RequestSet[2].Value);
                }
                else
                {
...
>>>>>>> parent of bcab9ee... Forex2.0 Переписать код под более строгую архитектуру ООП, создать БД.
        ...
        private void Button10_Click(object sender, EventArgs e)
        {
            Methods se = new Methods();
<<<<<<< HEAD
            Deal Sdelka = new Deal(false, Buffer, tic);
            // Запомнить значение продажи   
            BUY.Add(Sdelka);   
=======
...
>>>>>>> parent of bcab9ee... Forex2.0 Переписать код под более строгую архитектуру ООП, создать БД.
        ...
        private void LineCoord_CheckedChanged(object sender, EventArgs e)
        {
            Setting();
        }
        ...
        private void MimMaxCheckedChanged(object sender, EventArgs e)
        {
