    ...
    {//  
      ...
      public  void CreateFile(string pathFile)
        ...
        { 
            ...
            Console.WriteLine(massYInetB[(colvo/3)-1]);


            Methods cEURUSD = new Methods();// Метод конвертации времени
  ...
  public List<List<double>> IntervalResistance(int tic, List<double> massYInetBuy)//подавать интервал
        ...
        void chart1_MouseWheel(object sender, MouseEventArgs e)
        {
            if (e.Delta / 120 > 0)
            {
                switch (Leaves)
                {
                    case 1: Zoom -= 5; if (Zoom > 60) { ZoomT(Zoom); } break;
                }
            }
            ...
            if (e.Delta / 120 < 0)
            {
                switch (Leaves)
                {
                    case 1: Zoom += 5; ZoomT(Zoom); break;
                }
            }// прокрутка вниз
        }
